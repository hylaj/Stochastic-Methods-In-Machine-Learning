...
def new_metaheuristic(
    ...
    D = len(bounds)
    pop_size = 50 # Changed from 20 to 50
    ...
    F = 0.8  # Changed from 0.5 to 0.8
    CR = 0.2 # Changed from 0.9 to 0.2
    ...
    while budget_used < budget:
        for i in range(pop_size):
            ...
            # Ensure there are enough distinct indices for a, b, c
            if len(idxs) < 3: 
                # Fallback or break if population is too small for DE/rand/1
                # For this problem, pop_size=50 is large enough
                # and this check is mostly for robustness in edge cases.
                break 
